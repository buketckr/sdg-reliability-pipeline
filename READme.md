...
The function returns a standard Python dictionary containing:

- Final SDG evaluations
- Reliability information
- Pipeline metadata
- Token usage
- LLM response-time information
- Diagnostic information for the individual evaluation runs

No intermediate output files are required for integration.
...
- Response-time measurement for each LLM call
...
The purpose of the reliability pipeline is not to determine whether an SDG classification is objectively correct. Instead, it measures how consistently the LLM produces the same SDG category when the same course information is evaluated multiple times.
...
It should not be interpreted as the probability that the SDG classification is correct.
...
The pipeline aggregates these values separately for Stage 1, Stage 2, and the complete evaluation.

If Stage 2 is not performed, Stage 2 token values are zero.

---

## Timing Information

The pipeline records the LLM response time for every API call.
...
These values represent accumulated LLM response time, not a complete wall-clock benchmark of the surrounding application.

---

## Diagnostics

The returned dictionary contains a `diagnostics` section intended for development, integration testing, and troubleshooting.

It contains:

- Individual Stage 1 run scores
- Individual Stage 1 response times
- Per-run token usage
- SDGs that triggered Stage 2
- Near-boundary Stage 1 results
- Individual Stage 2 run scores
- Detailed Stage 2 reliability results

A frontend does not need to display diagnostic information in the default user interface.
...
For SDGs that require Stage 2, an additional reliability level may also be available.
...
An expandable details section may include:
...
Development or administrative interfaces may also use diagnostic information to inspect individual LLM runs, token usage, and response times.
...
Stage 1 reliability analysis
    ...
    +-------------------------------+
    |                               |
    | Stable / low-level variation  | Meaningful category disagreement
    |                               |
    v                               v
Resolved in Stage 1           3 additional LLM evaluations
                                    |
                                    v
                              Stage 2 analysis
                                    |
                 ...
                 stable_after_recheck     unstable
                          ...
                          +---------+---------+
                           ...
                           Stage 3 aggregation
                                    |
                                    v
                         Final SDG1-SDG16 result
                                    |
                         +----------+----------+
                         |          |          |
                         v          v          v
                    Reliability   Tokens     Timing
                                    |
                                    v
                               Diagnostics
```
...
No intermediate files are required by the pipeline.
...
The test performs real LLM API calls and therefore consumes API tokens.
